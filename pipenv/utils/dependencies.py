...
def python_version(path_to_python: str) -> Optional[str]:
...
def translate_markers(
    pipfile_entry: dict[str, Union[str, list[str], String]]
) -> dict[str, Union[str, list[str], String]]:
...
def unearth_hashes_for_dep(
    project: Project, dep: dict[str, Union[str, String]]
) -> list[Any]:
    hashes = []
...
def clean_resolved_dep(
    project: Project,
    dep: dict[str, Union[str, list[str], String]],
    is_top_level: bool = False,
    current_entry: Optional[dict[str, String]] = None,
) -> dict[
    str,
    Union[
        dict[str, str], dict[str, Union[str, String]], dict[str, Union[str, list[str]]]
    ],
]:
...
def is_star(val: Union[str, String]) -> bool:
...
def is_editable_path(path: Union[String, str]) -> bool:
...
def dependency_as_pip_install_line(
    ...
    include_index: bool,
    indexes: list[Union[Any, dict[str, Union[String, bool]]]],
...
def convert_deps_to_pip(
    deps: Any,
    indexes: Optional[list[dict[str, Union[String, bool]]]] = None,
...
def parse_cfg_file(content: str) -> None:
...
def determine_path_specifier(package: InstallRequirement) -> None:
    if package.link:
...
def determine_vcs_specifier(package: InstallRequirement) -> Optional[str]:
...
def find_package_name_from_filename(filename: str, file: BufferedReader) -> Optional[str]:
...
) -> Union[tuple[InstallRequirement, None], tuple[InstallRequirement, str]]:
...
def normalize_vcs_url(
    vcs_url: Union[String, str]
) -> Union[tuple[String, str], tuple[str, str]]:
...
def install_req_from_pipfile(
    name: str, pipfile: dict[str, Union[str, String]]
...
def get_constraints_from_deps(
    deps: dict[
        str,
        Union[dict[str, str], dict[str, list[Any]], dict[Any, Any], dict[str, list[str]]],
    ]
) -> set[str]:
...
def is_required_version(
    version: str,
    specified_version: Union[dict[str, Union[str, list[str]]], dict[str, str], str],
) -> bool:
